<script lang="ts">
	...
	import { BUILTIN_TEMPLATES, CATEGORY_NAME_KEY, TEMPLATE_CATEGORIES } from '$lib/domain/templates';
...
{#each TEMPLATE_CATEGORIES as category (category)}
	<h2 class="mt-4 mb-2 text-xs font-medium tracking-wide text-ink-faint uppercase first:mt-0">
		{t(CATEGORY_NAME_KEY[category])}
	</h2>
	<div class="flex flex-wrap gap-2">
		{#each BUILTIN_TEMPLATES.filter((x) => x.category === category) as tmpl (tmpl.templateId)}
